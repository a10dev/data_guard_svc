...
#define MSG_COMMANDS		"Management:\tlogin, logout, has_password, set_password, is_password, login, logout, change_password\n"\
	...
	"5. FLock-native:\t flock_info, flock_register, flock_suspend, flock_resume, flock_unregister, flock_let_unload, flock_set_dbgprint, flock_present, flock_mark_file, flock_read_meta, flock_kill, "\
...
void flockGetInfo()
{
	...
	if (client.getDriverInfo(drvInfo) ) {
		...
		std::cout << "\n\tCache capacity: " << std::dec << drvInfo.cache.capacity;
		std::cout << "\n\tCache current size: " << std::dec << drvInfo.cache.currentSize;
		std::cout << "\n\tCache occupancy limit: " << std::dec << drvInfo.cache.occupancyLimit;
		std::cout << "\n\tCache collisions occurrences: " << std::dec <<  drvInfo.cache.collisionOccurrences;
		std::cout << "\n\tCache collision max resolve offset: " << std::dec <<  drvInfo.cache.collisionMaxResolveOffset;
		std::cout << "\n\tCache collision resolve space border: " << std::dec << drvInfo.cache.collisionResolveIfNoPlaceBorder;
...
void flockCacheEnable()
{
	...
	if (client.cacheEnable( strings::equalStrings(enable, "enable") )) {
		std::cout << "\n\tSuccess. Cache was disabled." << std::endl;
	} else {
		std::cout << "\n\tError - cache was not cleared.\n";
	}
}

void flockCacheResize()
{
	::driver::ClientFLock client(*gLogFlock);

	std::cout << "\nEnter positive value for cache 'capacity':\n";
	std::string cache_size = readline();
	std::cout << "\nEnter positive value for 'occupancy limit' (should be smaller than 'capacity'):\n";
	std::string occupancy_limit = readline();
	std::cout << "\nEnter positive value for 'no_border_limit' (recommended number between 500 - 1500):\n";
	std::string no_border_limit = readline();

	if (client.cacheReSize(std::stoul(cache_size), std::stoul(occupancy_limit), std::stoul(no_border_limit))) {
		std::cout << "\n\tSuccess. Cache was resized." << std::endl;
	} else {
		std::cout << "\n\tError - cache was not resized.\n";
	}
}
...
{
	::driver::ClientFLock client(*gLogFlock);
	std::cout << "\nRequest to suspend FLock.\n";
	if (client.suspend(true)) {
		std::cout << "\n\tSuccess. FLock suspended." << std::endl;
	} else {
		std::cout << "\n\tError - failed to suspend FLock.\n";
	}
}
...
{
	::driver::ClientFLock client(*gLogFlock);
	std::cout << "\nRequest to resume FLock.\n";
	if (client.suspend(false)) {
		std::cout << "\n\tSuccess. FLock resumed." << std::endl;
	} else {
		std::cout << "\n\tError - failed to resume FLock.\n";
	}
}
...
void flockLetUnload()
{
	...
	std::cout << "\nRequest to let driver has unload functionality (type 'enable' or 'disable'): ";
	std::string enable = readline();

	if (client.letUnload(strings::equalStrings(enable, "enable"))) {
		std::cout << "\n\tSuccess. Request to driver succeeded.\n" << std::endl;
	}
	else {
		std::cout << "\n\tError. Request to driver failed.\n";
	}
}
...
void DataGuardConsole()
{
	...
	h["flock_cache_resize"] = flockCacheResize;
	h["flock_ctx_enable"] = flockContextEnable;
	h["flock_ctx_reset"] = flockContextReset;
	h["flock_suspend"] = flockSuspend;
	h["flock_resume"] = flockResume;
	h["flock_let_unload"] = flockLetUnload;
