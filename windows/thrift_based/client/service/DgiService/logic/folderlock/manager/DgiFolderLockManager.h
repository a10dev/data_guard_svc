	...
	namespace folderlock
	{
		namespace manager
		{
			...
			public:
				...
				common::InternalStatus get(
					const std::wstring& _path /* x:\files\my.doc */,
					::logic::folderlock::storage::FLockObject& _outFlock
				);
				...
				common::InternalStatus remove(
					std::wstring _path /* c:\myown\private\files */
				);

				common::InternalStatus removeById(
					const std::string& _flockId /* AF098DC3100EACB23CDE59 */
				);

				common::InternalStatus changeState(
					std::wstring _path /* c:\myown\dx\salary.doc */,
					::logic::folderlock::storage::FLockState _newState
				);

				//
				//	Clears flocks list of current service application and do the same in driver.
				//
				...
				//
				// Deletes all flocks in driver's storage.
				//
				bool clearDriverStorage();
				...
				logic::common::InternalStatus mergeFlocksWithDriver();
			...
			private:
				...
				// User-mode application level flock's storage.
				::logic::folderlock::storage::Keeper m_storage_app;
			};
		}
	}
}
