	...
	namespace folderlock
	{
		namespace manager
		{
			...
			FileGuard::FileGuard(std::wstring _logfilepath) :
				...
				m_driver(m_log),
				m_storage_app(::logic::common::DgiEngine::getCryptor(), m_log, "flock.storage")
			{
				...
				if (getDriver().registerAsService()) {
			...
			FileGuard::~FileGuard()
			{
				...
				if (getDriver().unregisterAsService()) {
			...
			logic::common::InternalStatus FileGuard::add(::logic::folderlock::storage::FLockObject& _flock)
			{
				...
				//
				// We can't add same object twice! Verify by path.
				//
				if (m_storage_app.present(path)) {
				...
				//
				// The second verification: by ID.
				//

				bool idIsAlreadyUsed = m_storage_app.presentById(std::string((const char*)_flock.uniqueId, sizeof(_flock.uniqueId)));
				if ( idIsAlreadyUsed ) {
				...
				// if ((_flock.state == storage::FLock_Hidden) || (_flock.state == storage::FLock_HiddenAndLocked))
				{
					...
					if (!isVolume(lowLevelPath))
					{
						...
						m_log.print(std::string(__FUNCTION__) + ": info - (!) parent dir is a volume.");
					}
				}
				...
				if (getDriver().writeFileMeta(lowLevelPath, signature))
				{
					//
			...
			logic::common::InternalStatus FileGuard::present(const std::wstring& _path)
			{
				...
				if (!m_storage_app.isLoaded()){
					return ::logic::common::InternalStatus::Int_UnknownError;
				}

				if (m_storage_app.present(_path)){
			...
			logic::common::InternalStatus FileGuard::presentById(const std::string& _flockId)
			{
				...
				if (m_storage_app.presentById(_flockId) ){
					return ::logic::common::InternalStatus::Int_Success;
				}
			...
			logic::common::InternalStatus FileGuard::get(const std::wstring& _path /* x:\files\my.doc */, ::logic::folderlock::storage::FLockObject& _outFlock)
			{
				...
				if(m_storage_app.getInfo(_path, _outFlock))
				{
					//
			...
			logic::common::InternalStatus FileGuard::getAll(std::vector<::logic::folderlock::storage::FLockObject>& _flocks)
			{
				...
				if (m_storage_app.isLoaded()) {
					auto flocks = m_storage_app.getAll();
					flocks.swap(_flocks);
			...
			logic::common::InternalStatus FileGuard::removeById(const std::string& _flockId /* AF098DC3100EACB23CDE59 */)
			{
				...
				if ( m_storage_app.removeById(_flockId) )
				{
			...
			logic::common::InternalStatus FileGuard::changeState(std::wstring _path /* c:\myown\dx\salary.doc */, ::logic::folderlock::storage::FLockState _newState)
			{
                ...
                //
				if (m_storage_app.setState(_path, _newState))
				{
					// Success, guys!
                    ...
                    //
					auto driverResponse = mergeFlocksWithDriver();

					if (IntSuccess(driverResponse)) {
						m_log.printEx("%s: success - flocks list was updated in driver.", __FUNCTION__);
					} else {
			...
			logic::common::InternalStatus FileGuard::clear()
			{
				...
				if (m_storage_app.clear()) {
					m_log.printEx("%s: success - service flock storage erased.", __FUNCTION__);
				} else {
					m_log.printEx("%s: error - could not remove flock entries in a service part.", __FUNCTION__);
				}

				// Now it is time to delete in kernel.
				if (clearDriverStorage()) {
					m_log.printEx("%s: success - flocks list also was updated in driver.", __FUNCTION__);

					// Return success result only after erasing driver's storage.
					result = ::logic::common::InternalStatus::Int_Success;
				} else {
					m_log.printEx("%s: error - flocks were not deleted in driver storage.", __FUNCTION__);
				}
			...
			bool FileGuard::verifyDriverConnection()
			{
				...
				bool established = getDriver().canConnect();
				if (!established) {
			...
			logic::common::InternalStatus FileGuard::sendFlocksToDriver(bool _refreshCache)
			{
				...
				for (auto flock : flocks)
				{
					...
					m_log.print(std::string(__FUNCTION__) + ": send driver request - to add flock #" + std::to_string(nFlock) + " -> " + ::driver::ClientFLock::getDescription(lowLevelFlock));

					if (getDriver().storageAdd(lowLevelFlock)) {
			...
			bool FileGuard::clearDriverStorage()
			{
				m_log.print(std::string(__FUNCTION__));

				bool deleted_in_driver = getDriver().storageClear();
				if (deleted_in_driver) {
					m_log.print(std::string(__FUNCTION__) + ": success - cleared driver flock storage.");
				} else {
					m_log.print(std::string(__FUNCTION__) + ": error - could not clear driver's flocks.");
				}
				return deleted_in_driver;
			}

			logic::common::InternalStatus FileGuard::mergeFlocksWithDriver()
			{
				m_log.print(std::string(__FUNCTION__));
				...
				//
				if (!m_storage_app.isLoaded()) {
					m_log.print(std::string(__FUNCTION__) + ": error - service flocks storage is not loaded.");
					return ::logic::common::InternalStatus::Int_NotLoaded;
				}

				auto applicationFlocks = m_storage_app.getAll();
				...
				//
				std::map<std::string /* flock id */, DWORD /* access policy */ > commonFlockList;

				std::vector<::driver::FLOCK_STORAGE_ENTRY> currentDrvFlocks;
				if (!getDriver().storageGetAll(currentDrvFlocks)) {
				...
				for (auto drvFlock : currentDrvFlocks) {
					...
					commonFlockList[flock_id] = flockProtection;
				}
				...
				for (auto flock : commonFlockList) {
					::driver::FLOCK_STORAGE_ENTRY lowLevelFlock = { 0 };
					lowLevelFlock.version = 0;
					...
					auto flock_hex_str = strings::getHex(flock.first.c_str(), flock.first.size());

					m_log.print(
						std::string(__FUNCTION__) +
						": send driver request - to add flock #" +
						flock_hex_str +
						" -> " +
						::driver::ClientFLock::getDescription(lowLevelFlock)
					);

					if (getDriver().storageAdd(lowLevelFlock)) {
						m_log.print(std::string(__FUNCTION__) + ": success - #" + flock_hex_str + " flock was added.");
					} else {
						m_log.print(std::string(__FUNCTION__) + ": error - #" + flock_hex_str + " flock was not added.");
					}
				}
            ...
            bool FileGuard::isSupportedFs(std::wstring _path, bool& _supported)
            {
				...
				// Create new empty file in a root dir of verified volume.
				...
				// Close file and let it be used to another users.
                CloseHandle(hTempFile);

				//
				// Here we need to do an attempt: write test Extended Attributes to new created file.
				//

                ::driver::FLOCK_META fm = { 0 };
                UCHAR metaSignature[] = FLOCK_TEST_SIGNATURE;
                memcpy(fm.signature, metaSignature, sizeof(metaSignature));

				 // !! 
                if (_supported = getDriver().makeMetaInvalid(tempFilePath)) {
                    getDriver().makeMetaInvalid(tempFilePath);

					// Delete newly created file because it is not needed now.
                    DeleteFileW(tempFilePath.c_str());
                } else {
                    m_log.print(fn + ": warning - failed to write flock-meta, " + strings::ws_s(volume, CP_UTF8) + " volume file system is not supported.");
                }
			...
			logic::common::InternalStatus FileGuard::markParentDirHasFLocks(std::wstring _path)
			{
				...
				if (getDriver().readFileMeta(parentPath, oldFlockMeta)) {
				...
				if (!getDriver().writeFileMeta(parentPath, fmParentDirMark)) {
			...
			::driver::ClientFLock& FileGuard::getDriver()
			{
				return m_driver;
			}
			...
			bool FileGuard::refreshDrvCaches()
			{
				...
				if (!getDriver().cacheClear()) {
					res = false;
					m_log.print(std::string(__FUNCTION__) + ": error - failed to clear driver's cache.");
				}

				if (!getDriver().ctxReset()) {
					res = false;
					m_log.print(std::string(__FUNCTION__) + ": error - failed to reset driver contexts.");
				}
			...
			{
				bool res = false;
				if (res = m_storage_app.setPassword(_password)) {
					m_storage_app.load();
				}
				return res;
			}
			...
			bool FileGuard::ctrlEventPasswordChanged(::logic::common::MasterPassword _password, ::logic::common::MasterPassword _currentPassword)
			{
				...
				bool success = m_storage_app.reEncrypt(_password, _currentPassword);
				if (success) {
