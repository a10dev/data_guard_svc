...
namespace driver
{
	...
	bool ClientFLock::storageGetAll(std::vector<FLOCK_STORAGE_ENTRY>& _flocks)
	{
		...
		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_QUERY_LIST, &request, sizeof(request), pOutput, outputSize, &cbReturned, NULL );
		if (res) {
			if (res = (responseHeader->flockStatus == FLOCK_STATUS_SUCCESS)) {
				...
				if (responseHeader->flockStatus == FLOCK_STATUS_SMALL_BUFFER) {
					...
					if (pBuffer) {
						...
						res = DeviceIoControl(hDrv, IOCTL_FLOCK_QUERY_LIST, &request, sizeof(request), pBuffer, outputSize, &cbReturned, NULL );
						if (res) {
							if (res = (responseHeader->flockStatus == FLOCK_STATUS_SUCCESS)) {
	...
	bool ClientFLock::storageGetOne(std::string _flock16bytesId, FLOCK_STORAGE_ENTRY& _flock)
	{
		...
		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_STORAGE_QUERY_ONE, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
	...
	bool ClientFLock::storageAdd(FLOCK_STORAGE_ENTRY _flock)
	{
		...
		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_STORAGE_ADD, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else{
	...
	bool ClientFLock::storageSetFlags(std::string _flock16bytesId, bool _toSet, ULONG32 _flags)
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_STORAGE_UPDATE_FLAGS,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else{
	...
	bool ClientFLock::storageRemove(std::string _flock16bytesId)
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_STORAGE_REMOVE,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	bool ClientFLock::storagePresent(std::string _flock16bytesId)
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_STORAGE_PRESENT,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
	...
	bool ClientFLock::ctxReset()
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_CONTEXT_RESET,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	bool ClientFLock::ctxEnable(bool _toEnable)
	{
		...
		BOOL res = DeviceIoControl( hDrv, IOCTL_FLOCK_CONTEXT_ENABLE, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
				m_log.printEx("%s: succeeded.", __FUNCTION__);
			}
		} else {
			DWORD lerror = GetLastError();
			m_log.printEx("%s: error - DevIo failed with 0x%x last error.", __FUNCTION__, lerror);
	...
	bool ClientFLock::letUnload(bool _toUnload)
	{
		FLOCK_REQUEST_HEADER request = buildNewRequest();
		FLOCK_RESPONSE_HEADER response = { 0 };
		DWORD cdReturned = 0;
		...
		request.params.boolValue = _toUnload;

		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_ENABLE_UNLOADING, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	bool ClientFLock::suspend(bool _toPause)
	{
		FLOCK_REQUEST_HEADER request = buildNewRequest();
		FLOCK_RESPONSE_HEADER response = { 0 };
		DWORD cdReturned = 0;
		...
		request.params.boolValue = _toPause;

		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_SUSPEND, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	bool ClientFLock::getDriverInfo(FLOCK_COMMON_INFO& _info)
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_COMMON_INFO,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
			if (!(res = (response.header.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.header.flockStatus);
			} else {
	...
	bool ClientFLock::cacheClear()
	{
		...
		BOOL res = DeviceIoControl(hDrv,IOCTL_FLOCK_CACHE_CLEAR,&request,sizeof(request),&response,sizeof(response),&cdReturned,NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	bool ClientFLock::cacheReSize(ULONG _capacity, ULONG _occupancy_limit, ULONG _collision_resolve_no_place_border)
	{
#pragma pack(push, 1)
		struct REQUEST_TO_RESIZE_CACHE
		{
			FLOCK_REQUEST_HEADER header;
			FLOCK_REQUEST_RESIZE_CACHE resize_info;
		};
#pragma pack(pop)

		
		REQUEST_TO_RESIZE_CACHE request = { 0 };
		request.header = buildNewRequest();
		request.header.length = sizeof(FLOCK_REQUEST_RESIZE_CACHE);
		FLOCK_RESPONSE_HEADER response = { 0 };
		DWORD cdReturned = 0;
		HANDLE hDrv = getDriverHandle();
		if (!hDrv) {
		...
		request.resize_info.capacity = _capacity;
		request.resize_info.occupancyLimit = _occupancy_limit;
		request.resize_info.collisionResolveIfNoPlaceBorder = _collision_resolve_no_place_border;

		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_CACHE_RESIZE, &request, sizeof(request), &response, sizeof(response), &cdReturned, NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
				m_log.printEx("%s: success.", __FUNCTION__);
				if (response.params.boolValue) {
					m_log.printEx("%s: internal flock cache was resized.", __FUNCTION__);
				} else {
					m_log.printEx("%s: but internal flock cache was not resized....", __FUNCTION__);
				}
			}
		} else {
			DWORD lerror = GetLastError();
	...
	bool ClientFLock::cacheEnable(bool _toEnable)
	{
		...
		BOOL res = DeviceIoControl(hDrv, IOCTL_FLOCK_CACHE_ENABLE, &request, sizeof(request), &response, sizeof(response), &cdReturned,NULL);
		if (res) {
			if (!(res = (response.flockStatus == FLOCK_STATUS_SUCCESS))) {
				m_log.printEx("%s: flock error is 0x%x.", __FUNCTION__, response.flockStatus);
			} else {
	...
	std::string ClientFLock::getDescription(const FLOCK_STORAGE_ENTRY& _flock_entry)
	{
		std::stringstream ss_hex_id;
		const int id_size = sizeof(FLOCK_STORAGE_ENTRY::id);
		ss_hex_id << std::hex;
		for (int i(0); i < id_size; ++i) {
			ss_hex_id << std::setw(2) << std::setfill('0') << (int)_flock_entry.id[i];
		}
		return (std::string("flock_v") + std::to_string(_flock_entry.version) + " id " + ss_hex_id.str() + " flg_" + std::to_string(_flock_entry.flockFlag));
	}
}
