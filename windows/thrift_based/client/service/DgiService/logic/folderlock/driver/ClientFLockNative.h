...
#define FLOCK_TEST_SIGNATURE			{0x11, 0x2E, 0x33, 0xf4, /*1*/ 0xb2, 0x1E, 0x27, 0x21, /*2*/ 0x00, 0x00, 0x00, 0x00 /*3*/, 0x28, 0x93, 0x77, 0x45 /*4*/}
...
// Suspends or resumes file system access control management activity.
#define IOCTL_FLOCK_SUSPEND					CTL_CODE(FLOCK_DEVICE, 0x0810, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#pragma pack(push, 1)
	...
	typedef struct _FLOCK_REQUEST_RESIZE_CACHE
	{
		ULONG capacity;
		ULONG occupancyLimit;
		ULONG collisionResolveIfNoPlaceBorder;
	} FLOCK_REQUEST_RESIZE_CACHE, *PFLOCK_REQUEST_RESIZE_CACHE;
...
#pragma pack(pop)
	...
	public:
		...
		bool letUnload(bool _toEnable);

		bool suspend(bool _toPause);

		bool getDriverInfo(FLOCK_COMMON_INFO& _info);

		bool cacheClear();

		bool cacheReSize(ULONG _capacity, ULONG _occupancy_limit, ULONG _collision_resolve_no_place_border);

		bool cacheEnable(bool _toEnable);

		bool cacheGetInfo(/* Return info here in arguments. */);

		static std::string getDescription(const FLOCK_STORAGE_ENTRY& _flock_entry);
